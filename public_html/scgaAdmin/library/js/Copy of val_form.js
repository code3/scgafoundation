...
*/
 ...
 valForm.failed = true; valForm.stopSubmit = false; valForm.ajaxRunning = new Object(); if(valForm.inputs){ for(var i=0; i<valForm.inputs.length; i++){

 Event.stopObserving(valForm.inputs[i], 'blur', valForm.inputObsFuncs[i]); } Event.stopObserving(valForm.form, 'submit', valForm.formObsFunc); 
...
if(Vf9b843c0.length == 0){ alert('valForm init error: no submit button'); } else{ valForm.submitBtn = Vf9b843c0[0];
 ...
 var Vccaee621 = valForm.form.select('input[type="text"]'); var Vc836810e = valForm.form.select('input[type="password"]');

var Vb954ef8a = valForm.form.select('input[type="file"]'); var V31b9feb5 = valForm.form.select('select');  var Vkeilmdiowd2 = valForm.form.select('textarea');
